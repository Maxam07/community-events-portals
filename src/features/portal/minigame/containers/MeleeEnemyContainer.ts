...
import type { DamagePayload, MeleeEnemyTypes } from "../Types";
import type { EnemyConfig } from "../Types";
import { MELEE_CONFIGS } from "../constants/EnemyConstants";
...
export class MeleeEnemy extends Phaser.GameObjects.Container {
  scene: Scene;
  ...
  constructor({ scene, x, y, player, mobType }: Props) {
    ...
    this.config = MELEE_CONFIGS[mobType];
  ...
  private createAnim() {
    this.walkAnimKey = `${this.config.key}_anim`;
    ...
    // Attack animation is optional
    if (this.config.attackKey) {
      this.attackAnimKey = `${this.config.attackKey}_attack`;

      if (!this.scene.anims.exists(this.attackAnimKey)) {
        this.scene.anims.create({
  ...
  public updateMovement(delta: number) {
    ...
    const attackRange = 0.5 * SQUARE_WIDTH;
  ...
  public onDeath() {
    ...
    this.scene.handleMeleeMobDefeat(this);
  }
}
